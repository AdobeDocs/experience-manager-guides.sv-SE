...
| Utdatasökväg/målsökväg | Sökvägen i AEM-databasen där PDF lagras.<br>Utdatasökvägen ställs in med variabeln `${base_output_path}` som konfigureras av administratören. Om du vill konfigurera utdatasökvägen kan du visa [Konfigurera basutdataplats för molntjänster](../native-pdf/configure-base-location-cs.md) eller [Konfigurera basutdataplats för lokala tjänster](../native-pdf/configure-base-output-location.md) baserat på de tjänster du använder.<br> Du kan också använda vissa variabler som inte finns i kartongen för att definiera utdatasökvägen, visa [Använd variabler för att ange alternativen Målsökväg, Platsnamn eller Filnamn](generate-output-use-variables.md#id18BUG70K05Z). |
