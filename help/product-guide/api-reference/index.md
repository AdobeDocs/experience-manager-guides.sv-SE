---
title: API-referens för Adobe Experience Manager-guider
description: API-referens för Adobe Experience Manager-guider
source-git-commit: 5e0584f1bf0216b8b00f00b9fe46fa682c244e08
workflow-type: tm+mt
source-wordcount: '110'
ht-degree: 0%

---


# API-referens för Adobe Experience Manager-guider

- [Ansvarsfriskrivning](rebranding-disclaimer.md)
- [Introduktion](introduction.md)
- [REST API:er för utdatahantering](output-management.md)
- [REST API för arbete med DITA-kartor](dita-map-management.md)
- [REST API för att registrera en datakällkoppling](data-source-connector.md)
- [REST API för att arbeta med villkorsattribut](conditional-attribute.md)
- [REST API:er för konverteringsarbetsflöde](convert-to-dita.md)
- [REST API för att skapa och aktivera paket](create-activate-package.md)
- [Java-baserat API som fungerar med utdatagenerering](generate-output.md)
- [Java-baserade API:er för DITA-kartor](dita-map-management-java.md)
- [Java-baserade API:er för konverteringsarbetsflöde](convert-to-dita-java.md)
- [Java-baserade API:er som fungerar med baslinje och etiketter](create-baseline-label-java.md)
- [Java-baserat API för att arbeta med mappprofiler](folder-profile-java.md)
- [Java-baserat API för att skapa och aktivera paket](create-activate-package-java.md)
- [Händelsehanterare efter bearbetning](post-process-event.md)
- [Händelsehanterare för konverteringsprocess](conversion-complete-event.md)

