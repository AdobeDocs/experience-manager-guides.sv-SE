...
Ett ändringsfält är en lodrät linje som visuellt identifierar nytt eller ändrat innehåll. I AEM Guides kan du visa ett ändringsfält till vänster om det ändrade innehållet i ämnen och även de ändrade ämnena i innehållsförteckningen för PDF-utdata.

Mer information om hur du visar ändringsfältet finns i *Skapa PDF med ändringsfältet mellan publicerade versioner* i [Publicera PDF-utdata](../web-editor/native-pdf-web-editor.md).
...
Du kan till exempel använda färgattributet i formatet `.inserted-block` för att definiera hur det infogade innehållet ska visas i publicerade PDF-utdata.
...
Du kan också lägga till ett ändringsfält till vänster om de ändrade avsnitten i innehållsförteckningen för PDF-utdata. Du kan använda attributet `-ro-change-bar-color` i formatet `.changed-topic` för att lägga till ett ändringsfält i den färg du väljer för de uppdaterade avsnitten i innehållsförteckningslistan.
