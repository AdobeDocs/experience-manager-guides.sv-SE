...
Den här versionsinformationen innehåller uppgraderingsinstruktioner, kompatibilitetsmatris och problem som åtgärdats i version november 2023 av Adobe Experience Manager Guides as a Cloud Service (kallas senare *Experience Manager Guides as a Cloud Service*).
...
2. Uppdatera egenskapen `<dox.version>` i `/dox/dox.installer/pom.xml`-filen för Git-koden för molntjänster till 2023.11.0.406.
3. Genomför ändringarna och kör Cloud Services-pipeline för att uppgradera till november 2023-utgåvan av Experience Manager Guides as a Cloud Service.
...
- När du stänger ett avsnitt efter att du har redigerat det omdirigeras du till AEM hemsida i stället för att gå tillbaka till mappvyn. (13306)
...
- Problem vid implementering av datalager för en stor DITA-karta med scope-peer-länkar i AEM Site publishing. (13530)
- I PDF-publicering äventyras tillgängligheten eftersom bilder i sidhuvudet och sidfoten inte visar alternativ text. (12829)
- Det går inte att duplicera sidlayout i PDF utan att lägga till något tillägg automatiskt. (12534)
- När du genererar PDF-utdata med PDF-publicering kortas filnamnet av efter en viss period. (13620)
- Felaktiga ikoner och verktygstips visas för alternativet **Redigera innehåll** i verktygsfältet Sidlayouter i mallarna som används vid PDF-publicering. 13492
...
- I förinställningen för AEM-plats är alternativet att **generera separata PDF för varje avsnitt** inte funktionellt. (11555)
- PDF saknar stöd för CMYK-färgmodellskonvertering. (10754)
...
- Fliken **Rapporter** i webbredigerarens användargränssnitt kan inte visa ämneslistan för gamla DITA-kartor som skapats före uppgraderingen av Experience Manager Guides as a Cloud Service från juli 2023. (11852)
...
Adobe har identifierat följande kända fel i november 2023-versionen.

- Selektiv omgenerering av ämnen för utdata från AEM Site misslyckas.
